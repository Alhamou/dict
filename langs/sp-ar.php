...
function ad_to_head()
{
 ...
 <body>
  ...
  <option class="option-lang" value="sp">Spanish</option>

  </select>
  </form>
  <img src="https://alhamou.com/img/flags/es.svg" alt="Spanish" class="sllim">
...
$sql_1 ="SELECT * FROM  word_spain WHERE arabic_word_fl LIKE '$word' OR spain_word LIKE '$word' or arabic_word LIKE '$word' LIMIT 10 ";
$sql_2 ="SELECT * FROM  word_spain WHERE arabic_word_fl LIKE '$word %' OR spain_word LIKE '$word %' ORDER BY LENGTH(spain_word) DESC  LIMIT 4 ";
$sql_3 ="SELECT * FROM  word_spain WHERE arabic_word_fl LIKE '% $word' AND arabic_word_fl NOT LIKE '$word' OR spain_word LIKE '% $word' AND spain_word NOT LIKE '$word' LIMIT 4 ";
...
if($leng_1 > 0){while($row = $stmt_1->fetch()) {
  ...
  $spakwordde = $row["spain_word"];
  $spakwordar = $row["arabic_word_fl"];
  $wordurl = "https://alhamou.com/swen.php?langs=sp&word=".$row["spain_word"];
  $wordur2 = "https://alhamou.com/swar.php?langs=sp&word=".$row["arabic_word_fl"];


  include ('xml-result/sp-ar-result.php');};};
///////////////////////// END 1
...
if($leng_3 > 0){while($row = $stmt_3->fetch()) {
  ...
  $sql_4 ="SELECT * FROM  word_spain WHERE arabic_word_fl LIKE '%$word%' AND arabic_word_fl NOT LIKE '$word' OR spain_word LIKE '%$word%' AND spain_word NOT LIKE '$word' LIMIT 4 ";
...
if ($e >= 1) { } else { // STAR IF CONDITION
  ...
  $ssmean = $conn->query('SELECT * FROM word_spain_mean');
  foreach( $ssmean as $term ) {
    ...
    foreach ($terms as $value) {
      ...
      if ($result_find >=80) {
        $spakwordde = $value["spain_word"];
        $spakwordar = $value["arabic_word_fl"];
        $wordur2 = "https://alhamou.com/swar.php?langs=sp&word=".$value["arabic_word_fl"];
        include ('mean/sp-ar-mean-result/mean-result-ar.php');
      };};

$ssmean = $conn->query('SELECT * FROM word_spain_mean');
foreach( $ssmean as $term ) {
  $terms[ $term['spain_word'] ] = $term;};
  foreach ($terms as $value) {
    $find = $word;
    $value_de = $value['spain_word'];
    $result_find = 0;
    similar_text($value_de,$find,$result_find);
    if ($result_find >=80) {
      $spakwordde = $value["spain_word"];
      $spakwordar = $value["arabic_word_fl"];
      $wordur2 = "https://alhamou.com/swen.php?langs=sp&word=".$value["spain_word"];
      include ('mean/sp-ar-mean-result/mean-result-sp.php');
    };};
...
include ('include/ads.php') /* GOOGLE ADS */?>
 ...
 <script>
  ...
  source: 'https://alhamou.com/ajax/sp-ar-ajax.php' ,
  minLength : 1
  });
  });
  </script>
