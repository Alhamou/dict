...
<div class="satz">
  <div class="">
    ...
    <div class="enabicword">
      <i class="fa fa-volume-up " onclick='responsiveVoice.speak("<?php echo $spakwordde ?>","Spanish Female")'></i>
      <span class="article floatleft" ><?php echo $row["article"] ?></span>
      <a class="a-de" href='<?php echo $wordurl ?>'><span class="worden"><?php echo preg_replace("/\p{L}*?".preg_quote($word)."\p{L}*/ui","<span class='mark-ar-mark'>$0</span>", $row["spain_word"]) ?></span></a>
      <span class="discen"><?php echo $row["spain_descreption"] ?></span>
    </div>
  </div>
  <div class="reexbutton">
    <form  class="exform" action="https://www.alhamou.com/exswen.php" method="GET">
      <input type="hidden" name="langs" value="sp">
      <input type="hidden" name="examples" value="<?php echo $row["spain_word"]?>">
