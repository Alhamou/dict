...
<img src="img/flags/kr.svg" alt="Korea" class="sllim">
</div>
...
if (strlen($word) >1){// دالة شرطية بعدد حروف  كلمة البحث
  ...
  $qexamples = "SELECT * FROM korean_examples WHERE ko LIKE '% $word %'LIMIT 7 ";
  ...
  if ($wc >= 1 ){while ($row_ex = $stmt->fetch()){
    ...
    $qexamples = "SELECT * FROM korean_examples WHERE ko LIKE '%$word%'LIMIT 5 ";
