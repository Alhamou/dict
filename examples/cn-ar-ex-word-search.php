...
if ($_SERVER['RECUEST_METHUD'] == 'GET'){
  ?>
<title>ترجمة كلمة <?php echo $word ?>في الأمثلة الصينية - قاموس الحمو</title>  <?php
} else {
echo " <title>إبحث عن كلمة في الأمثلة الصينية - قاموس الحمو</title>";
};
...
<img src="../img/flags/cn.svg" alt="Chine" class="sllim">
</div>
...
$word = trim($trip);// كلمة البحث المفلترة النهائية الجاهزة للبحث
  ...
  $qexamples = "SELECT * FROM  chine_examples WHERE ar LIKE '% $word %' OR cn LIKE '% $word %'LIMIT 7 ";
  ...
  $qexamples = "SELECT * FROM  chine_examples WHERE ar LIKE '%$word%' OR cn LIKE '%$word%'LIMIT 5 ";
