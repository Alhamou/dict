...
class Pagination
{
    ...
    */
    ...
    protected function build_links()
    {
         ...
         * Start the UL
         */
        $this->links_html = '"'.$this->options['class_ul'].'"'.PHP_EOL;
    ...
    public function build_li_element($class, $text, $is_link = false, $page_number = 0)
    {
        if($is_link == false)
        {
            $this->links_html .= '"'.$class.'"'.$text.''.PHP_EOL;
        }
        else
        {
            $this->links_html .= '"'.$class.'""'.$this->build_link_url($page_number).'"'.$text.''.PHP_EOL;
        }
    }
    ...
    protected function build_links_next_last()
    {
        ...
        if($this->options['show_links_prev_next'] == true)
        {
            ...
            {
                $this->links_html .= '"'.$this->options['class_dead_links'].'"'.$this->options['text_next'].''.PHP_EOL;
            }
            elseif($this->current_page != $this->total_pages)
            {
                $this->links_html .= '"'.$this->options['class_live_links'].'""'.$this->build_link_url($this->current_page + 1).'"'.$this->options['text_next'].''.PHP_EOL;
            }
        ...
        if($this->options['show_links_first_last'] == true)
        {
            ...
            {
                $this->links_html .= '"'.$this->options['class_dead_links'].'">'.$this->options['text_last'].''.PHP_EOL;
            }
            elseif($this->current_page != $this->total_pages)
            {
                $this->links_html .= '"'.$this->options['class_live_links'].'""'.$this->build_link_url($this->total_pages).'"'.$this->options['text_last'].''.PHP_EOL;
            }
