...
<title>ترجمة كلمة<?php echo $word ?> في اللغة  الروسية - قاموس الحمو</title>
</head>
<body>
<?php
include 'include/head-log.php';?>

<option class="option-lang" value="ru" selected>Arabic - Russian</option>
...
if (isset($_GET)&& $word!=""){
  ...
  $sql_1 ="SELECT * FROM word_russian WHERE russian_word LIKE '$word' LIMIT 10 ";
  $sql_2 ="SELECT * FROM word_russian WHERE russian_word LIKE '$word %' LIMIT 4 ";
  $sql_3 ="SELECT * FROM word_russian WHERE russian_word LIKE '% $word' AND russian_word NOT LIKE '$word' LIMIT 4 ";
  ...
  if($leng_1 > 0){while($row = $stmt_1->fetch()) {
    ...
    $wordurl = "swen.php?langs=ru&word=".$row["russian_word"];
    $wordur2 = "swar.php?langs=ru&word=".$row["arabic_word_fl"];
  ...
  if($leng_3 > 0){while($row = $stmt_3->fetch()) {
    ...
    $sql_4 ="SELECT * FROM  word_russian WHERE russian_word LIKE '%$word%' AND russian_word NOT LIKE '$word' LIMIT 4 ";
    ...
    foreach( $ssmean as $term ) {
      ...
      foreach ($terms as $value) {
        ...
        if ($result_find >=80) {
          ...
          $wordurl = "swen.php?langs=ru&word=".$value["russian_word"];
          include ('mean/ru-ar-mean-result/mean-result-ru.php');};};}; // END IF CONDITION
        ////////////////////////////// END SINILAR TEXT ///////////////////////////////////////
