...
<title>ترجمة كلمة <?php echo $word ?>في القاموس الطبي - قاموس الحمو</title>
</head>
<body>
<?php
...
if (isset($_GET)&& $word!=""){
  ...
  $sql_1 ="SELECT * FROM medicine WHERE english_word LIKE '$word' LIMIT 10 ";
  $sql_2 ="SELECT * FROM medicine WHERE english_word LIKE '$word %' LIMIT 4 ";
  $sql_3 ="SELECT * FROM medicine WHERE english_word LIKE '% $word' AND english_word NOT LIKE '$word' LIMIT 4 ";
  ...
  if($leng_1 > 0){while($row = $stmt_1->fetch()) {
    ...
    $wordurl = "swen.php?langs=mc&word=".$row["english_word"];
    $wordur2 = "swar.php?langs=mc&word=".$row["arabic_word_fl"];
  ...
  if($leng_3 > 0){while($row = $stmt_3->fetch()) {
    ...
    $sql_4 ="SELECT * FROM  medicine WHERE english_word LIKE '%$word%' AND english_word NOT LIKE '$word' LIMIT 4 ";
    ...
    foreach( $ssmean as $term ) {
      ...
      foreach ($terms as $value) {
        ...
        if ($result_find >=80) {
          ...
          $wordurl = "swen.php?langs=mc&word=".$value["english_word"];
          include ('mean/mc-ar-mean-result/mean-result-en.php');};};}; // END IF CONDITION
        ////////////////////////////// END SINILAR TEXT ///////////////////////////////////////
